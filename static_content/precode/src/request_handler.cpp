...
namespace http_handler {
    ...
    RequestType RequestParser(const std::string& req_target) {
        size_t pos_api = req_target.find("/api/");
        if (pos_api != std::string::npos) { // Если запрос начинается с /api/
            ...
            if (pos != std::string::npos) { //проверка на правильный префикс
                ...
                if (next_slash_pos != req_.length() && next_slash_pos != std::string::npos) {
                    ...
                    if (req_.find('/') == std::string::npos) {
                        return {std::string(req_), RequestTypeEnum::API};
                    } else {
            ...
            std::cout << "File name parser" << std::endl;
            return {req_target, RequestTypeEnum::STATIC_FILE};
        }
    }
    ...
    boost::json::value RequestHandler::PrepareResponceAPI(const std::string& req_, const model::Game& game_) {
...
http::response<http::string_body> RequestHandler::CreateResponseStringBody(const std::string& req, const model::Game& game_) {
    http::response<http::string_body> resp;
    try {
        http::status status;
        std::string text;
        boost::json::value response_body = PrepareResponceAPI(req, game_);
...
http::response<http::string_body> RequestHandler::CreateResponseBadReqNotAllowed(http::status status) {
    ...
    if (status == http::status::bad_request) {
        ...
        res.body() = boost::json::serialize(jsonArr);
        res.result(status);
        return res;
    } 
    if (status == http::status::method_not_allowed) {
        res.body() = "Invalid method"s;
        res.result(status);
