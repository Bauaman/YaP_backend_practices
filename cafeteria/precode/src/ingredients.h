...
public:
    ...
    void StartFry(GasCooker& cooker, Handler handler) {
        ...
        cooker.UseBurner([self = shared_from_this(), handler = std::move(handler)] {
            ...
            std::osyncstream(std::cout) << "Started frying sausage # " << self->GetId() << std::endl;
            handler();
        });
    }
    ...
    void StopFry() {
        ...
        std::osyncstream(std::cout) << "Stopped frying sausage# " << this->GetId() << std::endl;
        // Освобождаем горелку
...
public:
    ...
    explicit Bread(int id, net::io_context& io)
        : id_{id},
        io_{io} {
    }
    ...
    void StartBake(GasCooker& cooker, Handler handler) {
        ...
        if (baking_start_time_) {
            throw std::logic_error("Baking has already started");
        }
        ...
        cooker.UseBurner([self = shared_from_this(), handler = std::move(handler)] {
            ...
            std::osyncstream(std::cout) << "Started baking Bread# " << self->GetId() << std::endl;
            handler();
        });
    }
    ...
    void StopBaking() {
        ...
        std::osyncstream(std::cout) << "Stopped baking Bread# " << this->GetId() << std::endl;
        // Освобождаем горелку
        gas_cooker_lock_.Unlock();
    }
...
private:
    ...
    net::io_context& io_;
    GasCookerLock gas_cooker_lock_;
    boost::asio::steady_timer baking_timer_{io_, 1s};
