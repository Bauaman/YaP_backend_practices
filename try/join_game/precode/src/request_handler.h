        ...
        if ((req.method() == http::verb::get) || (req.method() == http::verb::head)) {
            try {
                ...
                if (req_.req_type == RequestType::API) {
                    auto handle = [self = shared_from_this(), send,
                        ...
                        try {
                            ...
                            return send(self->HandleAPIRequest(std::move(req), req_, send));
                        } catch (...) {
                ...
                return std::visit(
                                ...
                                HandleStaticFileRequest(std::move(req), req_, send));
            } catch (...) {
...
private:
    ...
    template <typename Body, typename Allocator, typename SendFunction>
    void HandleAPIRequest(http::request<Body, http::basic_fields<Allocator>>&& req, const RequestData& req_data, SendFunction send) {
    ...
    template <typename Body, typename Allocator, typename SendFunction>
    void HandleStaticFileRequest(http::request<Body, http::basic_fields<Allocator>>&& req, const RequestData& req_data, SendFunction send) {
        try {
